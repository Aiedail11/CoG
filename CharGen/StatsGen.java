...
public class StatsGen {   //FIND OUT WHY POINTS ISN'T RIGHT AT START OF NORMAL STAT CALCULATION CTRL F FOR 'POINTS'
   ...
   public void calcBaseStats() {
      ...
      if(customConfig) {
         if((faunus_any_bonus > 0 && race.equals("Faunus"))
         || (eladrin_arcdex_bonus > 0 && race.equals("Eladrin"))
         || (drow_arcdex_bonus > 0  && race.equals("Drow"))) {
   ...
   public void calcExtraStats() { //dummy, this runs too slow, instead create boolean failure variables to determine
   //what is rolled, or once it hits three, just increment the lowest stat as much as possible
   ...
   //should just add to base stats
     ...
     // int lastWisps;
      ...
      int secondRange = (endPriority+strPriority);
      int thirdRange = (endPriority+strPriority+arcPriority);
      int endHit = 0, strHit= 0, dexHit= 0, arcHit= 0; //dummy, these were test variables
     ...
     //  while(wisps>20 && /*failures < 4 && */ someStatPossible()) {
   //          statChoice = (int)(Math.random()*totalPriority) +1;
   //          failures = 0;
   //         // lastWisps = wisps;
   //          
   ...
   //          //ENDURANCE PROCESS
   //          if(statChoice >0 && statChoice <= endPriority) {
   //             //find what wisps would be if endurance incremented and set lookAhead to that calculation
   //             endHit++;
   //             lookAhead = calcWispCost(end);
   //             if (lookAhead < 0) {
   //                failures++;
   ...
   //          //STRENGTH PROCESS
   //          if(statChoice >firstRange && statChoice <= secondRange) {
   //             //find what wisps would be if strength incremented and set lookAhead to that calculation
   //             strHit++;
   //             lookAhead = calcWispCost(str);
   //             if (lookAhead < 0) {
   //                failures++;
   ...
   //          //ARCANA PROCESS
   //          if(statChoice >secondRange && statChoice <= thirdRange) {
   //             //find what wisps would be if arcana incremented and set lookAhead to that calculation
   //             arcHit++;
   //             lookAhead = calcWispCost(arc);
   //             if (lookAhead < 0) {
   //                failures++;
   ...
   //          }
   //          
   //          //DEXTERITY PROCESS  dummy it's not hitting dex (see debug)
   //          if(statChoice >thirdRange && statChoice <= (totalPriority)) {
   //             //find what wisps would be if dexterity incremented and set lookAhead to that calculation
   //             dexHit++;
   //             lookAhead = calcWispCost(dex);
   //             if (lookAhead < 0) {
   //                failures++;
   ...
   //          }
   //       
   //       }//end while loop for wisp calculation
   ...
   //returns the number of wisps remaining if this stat were increased
      ...
      if(desiredStat<=10)
      {
         ...
         System.out.println("Current: " + wisps + " Predicted: " + predictedWisps);
      }
      else
      {
         switch(curStat)
         {
            ...
            default:
               ...
               System.out.println("WARNING: exceeds currently programmed stat ranges - see calcWispCost(int curStat)");
               break;
         }
      }
