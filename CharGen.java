   ...
   public  void enterMode() {
     ...
     //reset all values
      ...
      if(log.logging){    
         ...
         log.println("Enter desired mode. Type 'help' for more information.");
      }
...
//       }
     // mode = modeHelper(input);
      if(input.equalsIgnoreCase("random"))
         ...
         mode = 0;
      else if(input.equalsIgnoreCase("dmpresets"))
         mode = 4;
      else if(input.equalsIgnoreCase("testing"))
         mode = 5;
      else if(input.equalsIgnoreCase("mode enable"))
         mode = 6;
      else if(input.equalsIgnoreCase("mode disable"))
         mode = 7;
//       if(mode==0)
//       { helper.enterMode(); }
   ...
   //add commands to the string 'allCommands', dummy
      String command = "";
      if(allCommands.equals("")) {
         
         log.println("Enter the desired fields to output, separated by commas" +
            "\n\t(use 'help' to list available fields)");
        
      
        // desiredFields = log.takeInput().toLowerCase(); old way of taking input
         String temp = log.takeInput().toLowerCase();
         command = temp.split(" ")[0];
         allCommands+= (" " + temp).toLowerCase();
      }
      if(repeating){ command = "start";}
      ...
      (repeating && allCommands.contains("name")))
      {
         ...
         if(!repeating){specific();}
        
      }
      if(command.equalsIgnoreCase("class") ||
      (repeating && allCommands.contains("class")))
      {
      ...
      (repeating && allCommands.contains("race")))
      {
      ...
      (repeating && allCommands.contains("stats")))
      {
         if(myRace.equals("") || repeating)
         {
      ...
      //else if{usingStats = false;}//this must be directly underneath .contains("stats")
   ...
   public void printCharacter() {
      ...
      if((!myName.equals("") && mode!=2) ||
         (allCommands.contains("name") && mode==2)){
      //if(!myName.equals("")){
         charString+="\nName: " + myName +
                  "\nOrigin of Name: " +ethnicity +
                  "\nGender: " + genderString;
      ...
      if((!myClass.equals("") && mode!=2) ||
         (allCommands.contains("class") && mode==2)){                  
      //  if(!myClass.equals("")){
         charString+="\n\nClass: " + myClass;
      }
                  
      if((!myRace.equals("") && mode!=2) ||
         (allCommands.contains("race") && mode==2)){
      // if(!myRace.equals("")){
         charString+="\nRace: " + myRace;
      }
      if(usingStats)
